...
真实公开数据指标位于 `_codex_tmp/metasleepguard_outputs/public_sleep_real_baseline/`。原 synthetic 指标仍仅用于 smoke，并在 manifest 中标记 `used_for_accuracy=no`。
"""
...
| `run.ps1 -Task submission-pack` | 生成初赛文档与视频素材 | `_codex_tmp/metasleepguard_submission/` |
...
- 输出路径：`_codex_tmp/metasleepguard_outputs/public_sleep_real_baseline/`。
...
"""
    ...
    dataset_output = (
        "`_codex_tmp/metasleepguard_outputs/public_sleep_real_baseline/` 与提交目录 `public_sleep_metrics/`。"
        if public_summary
...
- 输出产物：`{repo_root / '_codex_tmp/metasleepguard_outputs/metabci_component_status.json'}`。
...
def _manifest_rows(
    ...
    for directory, label in ((reports_root / "ten_min_quality_calibration", "10-minute"), (reports_root / "sixty_min_continuous_recording", "60-minute")):
        for name in ("report.html", "report.md", "summary.json"):
            add(directory / name, "generated report", label, "submission evidence", "generated_from_real", False, True, "formal real-data output")
        for path in sorted(directory.glob("*.csv")):
            add(path, "generated CSV", label, "quality/window evidence", "generated_from_real", False, True, "formal real-data output")
        for path in sorted((directory / "figures").glob("*.png")):
            add(path, "generated PNG", label, "visual evidence", "generated_from_real", False, True, "formal real-data output")
    for path in metric_outputs:
        add(path, "submission CSV", "verified report outputs", "metric evidence", "generated_from_real", False, True, "generated by metrics-export")
    ...
    for path in sorted((repo_root / "_codex_tmp/metasleepguard_outputs/metrics").glob("*.json")):
        try:
...
def _copy_public_baseline(repo_root: Path, metric_dir: Path) -> tuple[dict | None, list[Path]]:
    source = repo_root / "_codex_tmp/metasleepguard_outputs/public_sleep_real_baseline"
...
def generate_submission_materials(
    ...
    reports_root = Path(reports_root).resolve() if reports_root else repo_root / "_codex_tmp/metasleepguard_outputs/real_openbci_reports"
    submission_root = Path(submission_root).resolve() if submission_root else repo_root / "_codex_tmp/metasleepguard_submission"
    ...
    test_result_path = repo_root / "_codex_tmp/metasleepguard_outputs/test_results.json"
    ...
    if section in {"all", "metrics"}:
        ...
        outputs.append(_write_text(repo_root / "_codex_tmp/metasleepguard_outputs/current_runnable_tasks.md", _runnable_tasks_markdown(public_summary)))
        outputs.append(_write_text(repo_root / "_codex_tmp/metasleepguard_outputs/real_data_conclusion_for_submission.md", _real_conclusion(ten, sixty, window_88)))
        outputs.append(_write_text(repo_root / "_codex_tmp/metasleepguard_outputs/public_sleep_metrics/README.md", _public_sleep_readme(public_summary)))
