...
```powershell
.\run.ps1 -Task public-sleep-baseline -Dataset sleep-edf -DataRoot .\data\SleepEDF
.\run.ps1 -Task public-sleep-eval -Dataset sleep-edf -DataRoot .\data\SleepEDF
.\run.ps1 -Task cross-dataset-eval -SleepEdfRoot .\data\SleepEDF -IsrucRoot .\data\ISRUC
```
...
def _video_assets(video_dir: Path, ten: dict, sixty: dict, public_summary: dict | None = None) -> list[Path]:
    public_evidence = (
        f"{public_summary['n_subjects']} 名 Sleep-EDF 被试"
        if public_summary
        else "已验证的 Sleep-EDF 专家标签数据"
    )
    script = f"""# 3 分钟演示视频脚本
...
| 2:50-3:00 | 展示数据边界页 | OpenBCI 不证明分期准确率;当前准确率由 {public_evidence}验证,ISRUC 仅完成接口测试 |
"""
    storyboard = """# 分镜表
...
def generate_submission_materials(
    ...
    outputs: list[Path] = []
    public_summary_path = repo_root / "outputs/metasleepguard_outputs/public_sleep_real_baseline/summary.json"
    public_summary = _read_json(public_summary_path) if public_summary_path.exists() else None
    ...
    if section in {"all", "demo"}:
        outputs.extend(_video_assets(submission_root / "02_demo_video_assets", ten, sixty, public_summary))
