...
Python：`<conda-root>\envs\metabci\python.exe`
...
Python：`<conda-root>\envs\metabci_stim\python.exe`
...
## Main Commands
    ...
    .\run.ps1 -Task isruc-download -MaxSubjects 15 -Python $py
    .\run.ps1 -Task isruc-validation -MaxSubjects 15 -RequireModel random_forest -Python $py
    .\run.ps1 -Task isruc-validation -MaxSubjects 15 -RequireModel xgboost -Python $py
...
The verified ISRUC-Sleep evidence uses 15 Subgroup-I subjects and 12,661 valid epochs from the pinned NEMAR `nm000111 v1.0.1` release. RandomForest and XGBoost 3.1.3 results are retained separately because neither model dominates every internal or cross-dataset direction. See `docs/ISRUC_NEMAR_VALIDATION.md` for the complete same-protocol comparison.
